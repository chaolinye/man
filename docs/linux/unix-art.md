...
人类其实更善于肉眼观察数据而不是推导控制流程。数据比程序逻辑更易驾驭。

高级语言、数据驱动编程、代码生成器、特定领域的微型语言等这些工具的共同点是，它们都是提升代码生成层次从而使规格说明更简炼的方法。
...
重要的是把程序逻辑从硬编定的控制结构转移到数据中

要确保两个用不同语言编写的语法分析程序接受同一种语法也出了名地难。最好是其中一个作为另一个的过滤器。

Unix程序员非常习惯于编写解析器规格说明来生成分析程序，用于处理类似语言的标记；这样一来，它就是个捷径，可以认为所剩工作由配置结构某种通用的树遍历就可以完成。
...
Unix的程序和周边环境交流的方式多种多样。可以很方便地把这些方式分成
- 启动环境查询
- 交互通道

### 什么应是可配置的

无畏的Unix回答是“一切。只要可能，就建立机制而把策略决定权交给用户。

对于能够可靠地进行自动检测的东西，就不要提供配置开关。尽量用自动检测来减少配置开关的数量，或者在运行时不断尝试其它方法直到成功。
